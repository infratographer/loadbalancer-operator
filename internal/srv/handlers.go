...
func (s *Server) processEvent(messages <-chan *message.Message) {
	for msg := range messages {
		...
		if slices.ContainsFunc(m.AdditionalSubjectIDs, s.locationCheck) {
			lb := s.newLoadBalancer(m.SubjectID, m.AdditionalSubjectIDs)
...
func (s *Server) processChange(messages <-chan *message.Message) {
	for msg := range messages {
		...
		if err != nil {
			s.Logger.Errorw("unable to unmarshal change message", "error", err)
			msg.Nack()
		}
		...
		if slices.ContainsFunc(m.AdditionalSubjectIDs, s.locationCheck) {
			...
			if lb.lbType != typeNoLB {
				...
				case m.EventType == string(events.CreateChangeType) && lb.lbType == typeLB:
					s.Logger.Infow("stub for creating loadbalancer", "loadbalancer", lb.loadBalancerID.String())
				...
				case m.EventType == string(events.DeleteChangeType) && lb.lbType == typeLB:
					s.Logger.Infow("stub for deleting loadbalancer", "loadbalancer", lb.loadBalancerID.String())
				...
				default:
					s.Logger.Infow("stub for updating loadbalancer", "loadbalancer", lb.loadBalancerID.String())
